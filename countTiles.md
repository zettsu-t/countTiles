...
13牌をそれぞれ1..9で表現し、区切り文字、改行文字、C文字列終端文字を加えると16 bytesです。そのため文字列の生成を、XMMレジスタ上で行うことができます。文字列バッファは16 bytesアラインメントします。念のためXMMレジスタ2つ分確保した上で17文字目を0終端しておきますが、NUL終端を除いて16文字を超える場合も複数レジスタ分の領域を確保すればよいです。

1. 13牌を、下の桁(最も右を0桁目とする)から数えて、奇数桁の集まりと偶数桁の集まりで分ける。
1. それぞれの牌を、数字を表す文字'1'..'9'に変換する。'0' (0x30) を足せばよい。VPSHUFB命令にパターンとして値0を与えることで、あるXMMレジスタのすべてのbyteに0x30をコピーする。
1. 区切り文字(:)、改行文字(\n)、NUL(0)を文字列に追加する。これらのASCIIコードから0x30をあらかじめ引いておくと、数字と合わせて、VPADDB命令で足すことができる。VPADDB命令の加算はwraparoundするので、ある文字の桁あふれは他の文字に伝搬しない。
1. これらの文字列を、エンディアンを考慮してVPSHUFB命令で並び替え、VMOVDQA命令で文字列バッファに格納する。

文字を以下の通り並べ替えます。数字以外は、空いているところに入れればよいです。各数字は文字列の何文字目か(先頭は0)、右から何桁目か(先頭は0)、およびLSB側から順に0..15 (8 bytes単位)です。

転送後の文字について、真下(転送前の桁)に書いてある数字が示す文字の位置を見つけて、その文字の最下行に転送後の何文字目かを書き込みます。例えば転送後の1文字目は転送前の9桁目ですので、9文字目の列の一番下に1を書き込みます。このようにして得られる転送後の位置が、VPSHUFB命令に与えるパターンです。
