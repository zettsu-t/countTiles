...
n索が何枚あるか調べるのは、ビット[n * 5, n * 5 + 4]にある1を数えれば分かります。0索はありませんが、パディングとして常に0にしておきます。九索より左(ビット63..50)も常に0にしておきます。

### 牌の組

n索があるかどうか調べるのために、ビット1を数えるのは煩雑なので、ビット列中の1は常に右側(LSB側)に寄せるように正規化します。そうすることで、対子、刻子、順子をパターンマッチングで検索できます。
...
* 順子は123, .. , 789とは一致しますが、範囲外つまり0-1-2や8-9-10とは一致しません。これは1未満、10以上は常にビットが0だからです(これが0索を導入した理由です)。
...
* 改行などの区切り文字は、まとめて組み立てる
* 待ち形のsortとuniqueは、待ち形の文字列ではなくビット表現を用いる(対子、刻子、順子は5組しかないので)
* 上記のビットパターンで、待ちと完成形の両方を含むのは00011だけである
